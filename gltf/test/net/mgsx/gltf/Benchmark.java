...
public class Benchmark extends ApplicationAdapter {
	...
	public void run(final int iterations, final boolean trace, final boolean check) {
		...
		measure("G3DJ", iterations, trace, new Runnable() {
			@Override
			public void run() {
				Model g3dj = new G3dModelLoader(new JsonReader())
						.loadModel(Gdx.files.classpath("benchmark/four-spheres.g3dj"));
				if (check)
					assertConsistent(g3dj);
				g3dj.dispose();
			}
		});
		measure("G3DB", iterations, trace, new Runnable() {
			@Override
			public void run() {
				Model g3db = new G3dModelLoader(new UBJsonReader())
						.loadModel(Gdx.files.classpath("benchmark/four-spheres.g3db"));
				if (check)
					assertConsistent(g3db);
				g3db.dispose();
			}
		});
		measure("GLTF separated", iterations, trace, new Runnable() {
			@Override
			public void run() {
				SceneAsset assetSep = new GLTFLoader()
						.load(Gdx.files.classpath("benchmark/separated/four-spheres.gltf"));
				if (check)
					assertConsistent(assetSep.scene.model);
				assetSep.dispose();
			}
		});
		measure("GLTF embeded", iterations, trace, new Runnable() {
			...
			public void run() {
				...
				if (check)
					assertConsistent(assetEmb.scene.model);
				assetEmb.dispose();
			}
		});
		measure("GLB", iterations, trace, new Runnable() {
			...
			public void run() {
				...
				if (check)
					assertConsistent(assetBin.scene.model);
				assetBin.dispose();
			}
		});
	...
	protected void assertConsistent(Model model) {
		...
		final long expectedAttributes = VertexAttributes.Usage.Position | VertexAttributes.Usage.Normal
