...
class GLTFBinaryExporter {
	...
	public void export(GLTFImage image, Texture texture, String baseName) {
		...
		FrameBuffer fbo = new FrameBuffer(texture.getTextureData().getFormat(), texture.getWidth(), texture.getHeight(),
				false);
		fbo.begin();
	...
	public static void savePNG(FileHandle file, Pixmap pixmap) {
		if (Gdx.app.getType() == ApplicationType.WebGL) {
			...
			try {
				...
				Method pixmapIO_writePNG = ClassReflection.getMethod(pixmapIO, "writePNG", FileHandle.class,
						Pixmap.class);
