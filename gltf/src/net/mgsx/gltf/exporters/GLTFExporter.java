...
public class GLTFExporter {
	...
	/**
	 * sub class may override this method in order to implement some custom name
	 * mapping.
	 * 
	 * @param texture
	...
	/**
	 * convenient method to export a single mesh primitiveType can be any of OpenGL
	 * primitive: {@link com.badlogic.gdx.graphics.GL20#GL_POINTS},
	 ...
	 * {@link com.badlogic.gdx.graphics.GL20#GL_TRIANGLE_FAN}, etc..
	 */
	...
	private Array<Integer> exportNodes(GLTFScene scene, Iterable<Node> nodes) {
		...
		for (Node node : nodes) {
			...
			// transform, either a matrix or individual component (we use individual
			// components but it might be an option)
	...
	GLTFAccessor obtainAccessor() {
		...
		if (root.accessors == null)
			root.accessors = new Array<GLTFAccessor>();
	...
	private GLTFMesh obtainMesh() {
		...
		if (root.meshes == null)
			root.meshes = new Array<GLTFMesh>();
