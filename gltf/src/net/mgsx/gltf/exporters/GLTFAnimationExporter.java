...
class GLTFAnimationExporter {
	...
	abstract static class ChannelExporter<T> {
		...
		private void export(GLTFExporter base, GLTFAnimation a, int nodeID, Array<NodeKeyframe<T>> keyFrames,
				String chanName, Interpolation interpolation) {
			if (a.channels == null)
				a.channels = new Array<GLTFAnimationChannel>();
			GLTFAnimationChannel chan = new GLTFAnimationChannel();
			a.channels.add(chan);
			if (a.samplers == null)
				a.samplers = new Array<GLTFAnimationSampler>();
	...
	private static final ChannelExporter<Vector3> channelExporterVector3 = new ChannelExporter<Vector3>(3,
			GLTFTypes.TYPE_VEC3) {
		@Override
	...
	private static final ChannelExporter<Quaternion> channelExporterQuaternion = new ChannelExporter<Quaternion>(4,
			GLTFTypes.TYPE_VEC4) {
		@Override
		...
		return new ChannelExporter<WeightVector>(1, GLTFTypes.TYPE_SCALAR, count) {
			...
			protected void getOutput(FloatBuffer outputs, WeightVector value) {
				if (value instanceof CubicWeightVector) {
					 ...
					 * end of chapter : When used with CUBICSPLINE interpolation, tangents (ak, bk)
					 * and values (vk) are grouped within keyframes:
	...
	private void export(Animation animation) {
		...
		if (base.root.animations == null)
			base.root.animations = new Array<GLTFAnimation>();
		...
		for (NodeAnimation nodeAnim : animation.nodeAnimations) {
			...
			if (nodeAnim.translation != null) {
				channelExporterVector3.export(base, a, nodeID, nodeAnim.translation, "translation",
						translationInterpolation(nodeAnim));
			}
			if (nodeAnim.rotation != null) {
				channelExporterQuaternion.export(base, a, nodeID, nodeAnim.rotation, "rotation",
						rotationInterpolation(nodeAnim));
			}
			...
			if (nodeAnim instanceof NodeAnimationHack) {
				...
				if (nodeAnimMorph.weights != null) {
					...
					channelExporterWeights(count).export(base, a, nodeID, nodeAnimMorph.weights, "weights",
							nodeAnimMorph.weightsMode);
				}
			}
		}
	}
