...
public class FacedMultiCubemapData implements CubemapData {
	 ...
	 * 
	 * @param files
	 *            texture files in following order : level 0 (positive X, negative
	 *            X, positive Y, negative Y, positive Z, negative Z) level 1
	 *            (positive X, negative X, positive Y, negative Y, positive Z,
	 *            negative Z) and so on. Where level 0 is the biggest texture.
	 *            Expected levels x 6 files.
	 * @param levels
	 *            mipmap levels
	 */
		...
		for (int level = 0; level < levels; level++) {
			for (int i = 0; i < 6; i++) {
				...
				if (data[index].getType() == TextureData.TextureDataType.Custom) {
					...
					Gdx.gl.glTexImage2D(GL20.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, level, pixmap.getGLInternalFormat(),
