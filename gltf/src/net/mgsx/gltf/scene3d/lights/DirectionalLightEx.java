...
public class DirectionalLightEx extends DirectionalLight {
	...
	public DirectionalLight set(final DirectionalLight copyFrom) {
		if (copyFrom instanceof DirectionalLightEx) {
			return set(((DirectionalLightEx) copyFrom).baseColor, copyFrom.direction,
					((DirectionalLightEx) copyFrom).intensity);
		} else {
	...
	public boolean equals(DirectionalLightEx other) {
		return (other != null) && ((other == this) || ((baseColor.equals(other.baseColor)
				&& Float.compare(intensity, other.intensity) == 0 && direction.equals(other.direction))));
	}
}
