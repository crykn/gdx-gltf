...
public class AnimationControllerHack extends AnimationController {
	public static class Transform implements Poolable {
		...
		public Transform lerp(final Vector3 targetT, final Quaternion targetR, final Vector3 targetS,
				final WeightVector targetW, final float alpha) {
	 ...
	 * 
	 * @param weight
	 *            The blend weight of this animation relative to the previous
	 *            applied animations.
	 */
	@Override
	...
	private static void cubic(Vector3 out, float t, Vector3 p0, Vector3 m0, Vector3 p1, Vector3 m1) {
		...
		float t2 = t * t;
		float t3 = t2 * t;
		out.set(p0).scl(2 * t3 - 3 * t2 + 1).mulAdd(m0, t3 - 2 * t2 + t).mulAdd(p1, -2 * t3 + 3 * t2).mulAdd(m1,
				t3 - t2);
	}
	 ...
	 */
	private static void cubic(Quaternion out, float t, float delta, Quaternion p0, Quaternion m0, Quaternion p1,
			Quaternion m1) {

		// XXX not good, see
		// https://github.com/KhronosGroup/glTF-Sample-Viewer/blob/master/src/interpolator.js#L42
		delta = -delta;
	...
	private static void cubic(WeightVector out, float t, WeightVector p0, WeightVector m0, WeightVector p1,
			WeightVector m1) {
		// p(t) = (2t3 - 3t2 + 1)p0 + (t3 - 2t2 + t)m0 + (-2t3 + 3t2)p1 + (t3 - t2)m1
		float t2 = t * t;
		float t3 = t2 * t;
	...
	private final static Quaternion getRotationAtTime(final NodeAnimation nodeAnim, final float time,
		...
		if (interpolation == Interpolation.STEP) {
			...
			if (++index < nodeAnim.rotation.size) {
				...
				cubic(out, t, secondKeyframe.keytime - firstKeyframe.keytime, firstCV, firstCV.tangentOut, secondCV,
						secondCV.tangentIn);
			} else {
				out.set((Quaternion) firstKeyframe.value);
			}
	...
	private final static WeightVector getMorphTargetAtTime(final NodeAnimationHack nodeAnim, final float time,
			final WeightVector out) {
	...
	private final static void applyNodeAnimationBlending(final NodeAnimation nodeAnim,
		...
		if (t != null) {
			...
			if (alpha > 0.999999f)
				...
				out.put(node, pool.obtain().set(node.translation, node.rotation, node.scale, ((NodePlus) node).weights)
	 ...
	 */
	protected static void applyAnimationPlus(final ObjectMap<Node, Transform> out, final Pool<Transform> pool,
