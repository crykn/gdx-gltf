...
public class GLTFTypes {
	...
	public static int mapPrimitiveMode(Integer glMode) {
		if (glMode == null)
			return GL20.GL_TRIANGLES; // TODO not sure
		switch (glMode) {
		case 0:
	...
	public static Camera map(GLTFCamera glCamera) {
		if ("perspective".equals(glCamera.type)) {
			// see
			// https://github.com/KhronosGroup/glTF/tree/master/specification/2.0#perspectivezfar
			...
			float aspectRatio = glCamera.perspective.aspectRatio != null ? glCamera.perspective.aspectRatio
					: canvasRatio;
			float yfov = (float) Math.atan(Math.tan(glCamera.perspective.yfov * 0.5) * aspectRatio / canvasRatio)
					* 2.0f;
